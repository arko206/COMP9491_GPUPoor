...
from model.DISTAEformer import STAEformer
...
def train_one_epoch(
    ...
    for x_batch, y_batch in trainset_loader:
        x_batch = x_batch.to(DEVICE)
        y_batch = y_batch.to(DEVICE)
        out_batch, spurious = model(x_batch)
        out_batch = SCALER.inverse_transform(out_batch)
        spurious = SCALER.inverse_transform(spurious)
        spur_cri = nn.HuberLoss(reduction='none')
        loss = criterion(out_batch, y_batch)
        spur_loss = spur_cri(spurious, y_batch)
        spur_loss = spur_loss.transpose(0, 1).reshape(spur_loss.shape[1], -1).transpose(0, 1).mean(-1)
        loss = loss + la * (torch.var(spur_loss))
        batch_loss_list.append(loss.item())
